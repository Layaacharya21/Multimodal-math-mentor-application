...
# --- AUTOMATIC FLOW: SINGLE BUTTON FOR ALL STEPS ---
if st.button("Solve Problem", type="primary"):
    ...
    with st.spinner("Processing input, parsing problem, retrieving knowledge, and solving..."):
        # Step 1: Parse
        st.session_state.parsed_problem = parse_problem(input_text)
        
        if "error" in st.session_state.parsed_problem:
            st.error(st.session_state.parsed_problem["error"])
            st.stop()
        
        if st.session_state.parsed_problem.get("needs_clarification", False):
            st.warning("Problem is ambiguous — please edit input and retry.")
            st.stop()
    ...
    with st.expander("Verifier Report"):
        verdict = "Correct" if verification.get("is_correct") else "Uncertain/Incorrect"
